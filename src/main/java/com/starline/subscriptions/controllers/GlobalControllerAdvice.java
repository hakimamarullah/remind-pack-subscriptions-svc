...
import com.starline.subscriptions.annotations.LogResponse;
...
public class GlobalControllerAdvice {
    ...
    public ResponseEntity<ApiResponse<String>> feignExceptionHandler(FeignException ex) throws JsonProcessingException {
        log.error("Content Exception: {} Message: {}", ex.contentUTF8(), ex.getMessage());
        if (StringUtils.isBlank(ex.contentUTF8())) {
            return ApiResponse.setResponse(ex.getClass().getCanonicalName(), suppressMessage(ex.getMessage()), 500).toResponseEntity();
        }
